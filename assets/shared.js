(function(){
  ...
  function aiMount(){
    ...
    window.FUMI_AI_OPEN=(context,prompt="")=>{window.FUMI_AI_PINNED_CONTEXT=context||null;panel.classList.add("open");refresh();if(prompt)input.value=prompt;aiHealth().then(ok=>status.textContent=ok?"AI 已连接":"后台未连接或缺少密钥");setTimeout(()=>input.focus(),120)};
    fab.onclick=()=>{window.FUMI_AI_PINNED_CONTEXT=null;window.FUMI_AI_OPEN()};panel.querySelector(".ai-close").onclick=()=>panel.classList.remove("open");
